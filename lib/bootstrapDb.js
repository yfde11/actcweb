...
async function fixCertificateAttemptIndex() {
    ...
    // 刪除任何舊的 attempt_1（不管有沒有 unique/sparse），統一重建為非 unique sparse
    const oldAttempt = indexes.find((idx) => idx.name === 'attempt_1');
    if (oldAttempt && (oldAttempt.unique || !oldAttempt.sparse)) {
        await col.dropIndex('attempt_1');
        console.log('✅ Dropped old attempt_1 index on certificates (was unique or non-sparse)');
    }

    // 重新取清單後建立正確版本
    const fresh = await col.indexes();
    if (!fresh.some((i) => i.name === 'attempt_1')) {
        await col.createIndex({ attempt: 1 }, { sparse: true, name: 'attempt_1' });
        console.log('✅ Created sparse (non-unique) attempt_1 index on certificates');
    }
}
