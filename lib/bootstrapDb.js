...
async function fixCertificateAttemptIndex() {
    ...
    // 修正非 sparse 的 attempt_1（課程型憑證 attempt=null 會衝突）
    const indexes = await col.indexes();
    const badAttempt = indexes.find((idx) => idx.name === 'attempt_1' && !idx.sparse);
    if (badAttempt) {
        await col.dropIndex('attempt_1');
        console.log('✅ Dropped non-sparse attempt_1 index on certificates');
    }

    // drop 後重新取清單，確保重建判斷正確
    const freshIndexes = await col.indexes();
    const freshNames = freshIndexes.map((i) => i.name);

    if (!freshNames.includes('attempt_1')) {
        await col.createIndex({ attempt: 1 }, { unique: true, sparse: true, name: 'attempt_1' });
        console.log('✅ Created sparse unique attempt_1 index on certificates');
    }
    if (!freshNames.includes('user_1_exam_1')) {
        await col.createIndex({ user: 1, exam: 1 }, { name: 'user_1_exam_1' });
    }
    if (!freshNames.includes('issuedAt_-1')) {
        await col.createIndex({ issuedAt: -1 }, { name: 'issuedAt_-1' });
    }
    if (!freshNames.includes('certType_1_issuedAt_-1')) {
        await col.createIndex({ certType: 1, issuedAt: -1 }, { name: 'certType_1_issuedAt_-1' });
    }
    if (!freshNames.includes('isRevoked_1_expiresAt_1')) {
        await col.createIndex({ isRevoked: 1, expiresAt: 1 }, { name: 'isRevoked_1_expiresAt_1' });
    }
}
